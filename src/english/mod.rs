...
pub fn print_update_at_sub(date: &str){
    println!("# Updated at: {}", date);
}

pub fn print_type_exit_to_exit_sub() {
    println!("Type '{}' to terminate the program!", "exit".red());
    println!("");
}
...
pub fn print_level(level: u32) {
    if level == 1 {
        println!("== First Level, L1 ==");
    } else {
        println!("== Level {} L{} ==", level, level);
    }
}
...
pub fn print_what_num(question: u32, quantity: u32, highlight: &str) {
    print!("{}{} {}{}{} ",
        highlight.red(), 
        format!("{})",question).blue().bold(), 
        format!("What is the number ").yellow(), 
        quantity, 
        format!(" in English?").yellow());
}

pub fn print_correct() {
    println!("» {}","That's correct!".dimmed());
}

pub fn print_correct_ans(ans: &String, receive_ans: &String) {
    println!("» {}{}{}","'".dimmed(), receive_ans.italic(),"'.".dimmed());
    println!("» {}{}{}","The correct answer is '".dimmed(), ans.italic(),"'.".dimmed());
}
